...
from birdclef_2026.data.dataset import EmbeddingDataset, FixedWindowDataset, MixupDataset, OneHotLabelDataset, RandomWindowDataset
...
def build_combined_dataloaders(
    ...
    sl_val_indices, sl_train_indices_by_label = [], {}
    for label, group in sl_index.groupby("primary_label"):
        idx = group.index.to_numpy().copy()
        rng.shuffle(idx)
        n_val = min(max(1, int(len(idx) * val_fraction)), len(idx) - 1)
        sl_val_indices.extend(idx[:n_val])
        sl_train_indices_by_label[label] = idx[n_val:].tolist()

    max_count = max(len(v) for v in sl_train_indices_by_label.values())
    sl_train_indices = []
    for idx_list in sl_train_indices_by_label.values():
        n = len(idx_list)
        repeats, remainder = divmod(max_count, n)
        sl_train_indices.extend(idx_list * repeats + idx_list[:remainder])
...
def build_embedding_dataloaders(
    ...
    """Train/val DataLoaders for pre-computed embeddings.

    Splits by soundscape file to avoid leaking recordings across splits.
    Returns (train_loader, val_loader, label2idx).
    """
    index = pd.read_parquet(index_path)
    label2idx = _label2idx_from_taxonomy(taxonomy_path)
    ...
    rng = np.random.default_rng(seed)
    train_indices, val_indices = _split_by_file(index, val_fraction, rng)

    train_loader = DataLoader(
        OneHotLabelDataset(EmbeddingDataset(embeddings_path, index_path, train_indices), label2idx),
        batch_size=batch_size,
        shuffle=True,
        num_workers=4,
        pin_memory=True,
    ...
    val_loader = DataLoader(
        OneHotLabelDataset(EmbeddingDataset(embeddings_path, index_path, val_indices), label2idx),
        batch_size=batch_size,
        shuffle=False,
        num_workers=4,
        pin_memory=True,
